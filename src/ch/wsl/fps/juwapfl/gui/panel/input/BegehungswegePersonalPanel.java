 ...
 *
 */
public class BegehungswegePersonalPanel extends AbstractInputPanel implements CurrencySensitive {
	...
	private final JSpinner txtKostenMaschine = new JSpinner(new SpinnerNumberModel(DEFAULT_KOSTENSATZ_MASCHINE, 0, 1000, 1));
	...
	{
		AbstractMainWindow.adjustJSpinnerFormatter(txtKostenPersonal, false);
	...
	public BegehungswegePersonalPanel(AbstractMainWindow mainWindow) {
		super(Messages.getString("Begehungswege.TitelPersonalUndMaschinen")); //$NON-NLS-1$
		this.mainWindow = mainWindow;
		this.setLayout(new GridLayout(0, 2, 5, 0));

		this.add(lblKostenPersonal);
		this.add(Utilities.getPanelWithoutInfoButton(txtKostenPersonal));
	...
	private void initListeners() {
		txtKostenPersonal.addChangeListener(mainWindow.getDefaultChangeListener());
	...
	public Integer getKostenPersonal() {
		return (Integer) txtKostenPersonal.getValue();
	}
	
	public Integer getKostenMaschine() {
		return (Integer) txtKostenMaschine.getValue();
	}
	...
	public String getAsXmlString() {
		...
		sb.append("<section name=\"" + Messages.getString("Begehungswege.TitelPersonalUndMaschinen") + "\"> "); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$

		sb.append(getXmlEntry(Messages.getString("Common.Personalkostensatz") + " [" + mainWindow.getCurrency() + Messages.getString("Common.ProStunde") + "]", getKostenPersonal())); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
		sb.append(getXmlEntry(Messages.getString("Common.KostensatzMaschine") + " [" + mainWindow.getCurrency() + Messages.getString("Common.ProStunde") + "]", getKostenMaschine())); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
	...
	public void updateCurrency() {
		lblKostenPersonal.setText(Messages.getString("Common.Personalkostensatz") + " [" + mainWindow.getCurrency() + Messages.getString("Common.ProStunde") + "]");  //$NON-NLS-1$//$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
		lblKostenMaschine.setText(Messages.getString("Common.KostensatzMaschine") + " [" + mainWindow.getCurrency() + Messages.getString("Common.ProStunde") + "]");  //$NON-NLS-1$//$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
	}
